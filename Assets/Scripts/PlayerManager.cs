...
public class PlayerManager : CharacterManager
{
    ...
    private readonly float rotateSpeed = 180.0f;
    ...
    private void HandleAttackInput()
    {
        if (lightAttackInput)
        {
            attackManager.HandleLightAttackWeaponCombo(playerInventory.rightWeapon);
            lightAttackInput = false;
        }
        else if (heavyAttackInput)
        {
            heavyAttackInput = false;
            attackManager.HandleHeavyAttack();
        }
    }
    ...
    private void CameraRotation()
    {
        ...
        // calculate camera rotation on y axis
        cinemachineTargetYaw += mouseX * rotateSpeed * Time.deltaTime;

        // calculate camera rotation on x axis
        cinemachineTargetPitch += mouseY * rotateSpeed * Time.deltaTime;
        ...
        cinemachineTargetYaw += cameraAngleOverride;
        ...
        // rotate camera and target
        cinemachineCameraTarget.transform.rotation = Quaternion.Euler(0f, cinemachineTargetYaw, 0f);
        mainCamera.transform.rotation = Quaternion.Euler(cinemachineTargetPitch, cinemachineTargetYaw, 0f);
    }
